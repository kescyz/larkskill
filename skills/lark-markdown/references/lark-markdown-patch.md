...
```js
// 字面量替换
lark_api({ tool: 'markdown', op: 'patch', args: { file_token: 'boxcnxxxx', pattern: 'hello markdown', content: 'hello patched' } })

// 正则替换（RE2）
lark_api({ tool: 'markdown', op: 'patch', args: { file_token: 'boxcnxxxx', regex: true, pattern: 'hello (.+)', content: 'hi $1' } })

// 删除匹配内容
lark_api({ tool: 'markdown', op: 'patch', args: { file_token: 'boxcnxxxx', pattern: ' debug', content: '' } })

// pattern / content 也支持 @file
lark_api({ tool: 'markdown', op: 'patch', args: { file_token: 'boxcnxxxx', pattern: '@./pattern.txt', content: '@./replacement.md' } })
```
...
- 需要多组 patch 一次完成：当前不支持，改为多次调用 `lark_api({ tool: 'markdown', op: 'patch' })`
- 需要真正原子更新：当前能力不提供
