...
本文档提供基于 `lark_api({ tool: 'base', op: '...' })` 的完整示例。
...
```js
lark_api({ tool: 'base', op: 'table-create', args: {
...
```js
lark_api({ tool: 'base', op: 'record-upsert', args: {
  base_token: 'bascnXXXXXXXX',
  table_id: 'tblXXXXXXXX',
  json: {
    "客户名称":"字节跳动",
...
```js
lark_api({ tool: 'base', op: 'record-list', args: {
  base_token: 'bascnXXXXXXXX',
  table_id: 'tblXXXXXXXX',
  limit: 100
} })
...
- 需要一次性建表并附带字段、视图时，优先 `lark_api({ tool: 'base', op: 'table-create' })`
- 需要按业务字段名做 upsert 时，优先 `lark_api({ tool: 'base', op: 'record-upsert' })`
- 需要配置筛选视图时，优先 `lark_api({ tool: 'base', op: 'view-set-filter' })`
- 需要记录历史时，优先 `lark_api({ tool: 'base', op: 'record-history-list' })`
