...
```javascript
lark_api({ tool: 'base', op: 'form-detail', args: { share_token: '<share_token>', format: 'pretty' } })
```
...
```javascript
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', json: '{"fields":{"姓名":"张三","评分":5}}' } })
```

提交附件字段：

```javascript
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', base_token: '<base_token_from_form_detail>', json: '{"fields":{"姓名":"张三"},"attachments":{"附件":["./report.pdf"]}}' } })
```
