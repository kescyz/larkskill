...
| 你想做什么 | 用这些命令 | 关键文档 |
|------|-----------|---------|
| 创建/删除/改名称 | `lark_api({ tool: 'base', op: 'dashboard-create/delete/update' })` | 本页下方「仪表盘管理」 |
| 在仪表盘里添加组件 | `lark_api({ tool: 'base', op: 'dashboard-block-create' })` | 先读 [lark-base-dashboard-block-create.md](lark-base-dashboard-block-create.md)，再读 [dashboard-block-data-config.md](dashboard-block-data-config.md) |
| 修改组件 | `lark_api({ tool: 'base', op: 'dashboard-block-update' })` | 先读 [lark-base-dashboard-block-update.md](lark-base-dashboard-block-update.md)，再读 [dashboard-block-data-config.md](dashboard-block-data-config.md) |
| 查看仪表盘有哪些组件 | `lark_api({ tool: 'base', op: 'dashboard-get' })` 或 `lark_api({ tool: 'base', op: 'dashboard-block-list' })` | 本页下方「查看仪表盘」 |
| 智能重排组件布局 | `lark_api({ tool: 'base', op: 'dashboard-arrange' })` | [lark-base-dashboard-arrange.md](lark-base-dashboard-arrange.md) |
...
```js
// 第 1 步：创建空白仪表盘
lark_api({ tool: 'base', op: 'dashboard-create', args: { base_token: 'xxx', name: '销售数据分析' } })
// 记录返回的 dashboard_id

// 第 2 步：获取数据源信息
lark_api({ tool: 'base', op: 'table-list', args: { base_token: 'xxx' } })
lark_api({ tool: 'base', op: 'field-list', args: { base_token: 'xxx', table_id: 'tbl_xxx' } })
...
lark_api({ tool: 'base', op: 'dashboard-block-create', args: {
  ...
  name: '总销售额',
  type: 'statistics',
  data_config: {"table_name":"订单表","series":[{"field_name":"金额","rollup":"SUM"}]}
} })
...
lark_api({ tool: 'base', op: 'dashboard-block-create', args: {
  ...
  name: '月度趋势',
  type: 'line',
  data_config: {"table_name":"订单表","series":[{"field_name":"金额","rollup":"SUM"}],"group_by":[{"field_name":"月份","mode":"integrated"}]}
} })

// 继续创建其他组件...

// 第 5 步：组件创建完成后，使用 arrange 操作智能重排布局（可选但推荐）
...
lark_api({ tool: 'base', op: 'dashboard-list', args: { base_token: 'xxx' } })
// 获取目标 dashboard_id

// 第 2 步：根据用户诉求规划组件类型和数据源
// 建议先查看当前仪表盘已有组件，避免重复创建，或作为参考
lark_api({ tool: 'base', op: 'dashboard-get', args: { base_token: 'xxx', dashboard_id: 'blk_xxx' } })
...
> [!IMPORTANT]
> `lark_api({ tool: 'base', op: 'dashboard-block-update' })` **不能修改组件的 `type`**（图表类型），只能更新 `name` 和 `data_config`。
> 如需更换组件类型，必须先删除再重新创建。
...
// 第 2 步：列出组件，获取到目标组件
lark_api({ tool: 'base', op: 'dashboard-block-list', args: { base_token: 'xxx', dashboard_id: 'blk_xxx' } })
// 获取目标 block_id
// 提示：查看已有组件可作为参考，或检查是否重复创建相似组件

// 第 3 步：获取组件当前详情
lark_api({ tool: 'base', op: 'dashboard-block-get', args: { base_token: 'xxx', dashboard_id: 'blk_xxx', block_id: 'chtxxxxxxxx' } })
...
A:
1. 先读 [lark-base-dashboard-block-create.md](lark-base-dashboard-block-create.md) 了解 `name`、`type`、`data_config` 等参数
...
- 字段名拼写错误（必须用 `lark_api({ tool: 'base', op: 'field-list' })` 获取的真实字段名，禁止猜测）
- 组件创建并发执行（必须串行，等上一个完成再执行下一个）

**Q: 可以一次创建多个组件吗？**
A: 不可以，必须串行执行。等上一个 `lark_api({ tool: 'base', op: 'dashboard-block-create' })` 完成后再执行下一个。

**Q: 组件的 `type` 创建后能改吗？**
A: 不能。`lark_api({ tool: 'base', op: 'dashboard-block-update' })` 只能修改 `name` 和 `data_config`，不能修改 `type`。
