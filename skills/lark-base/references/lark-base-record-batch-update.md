...
```javascript
lark_api({ tool: 'base', op: 'record-batch-update', args: { base_token: '<base_token>', table_id: '<table_id>', json: '{"record_id_list":["<record_id>"],"patch":{"状态":"完成"}}' } })

lark_api({ tool: 'base', op: 'record-batch-update', args: { base_token: '<base_token>', table_id: '<table_id>', json: '@batch-update.json' } })
```
