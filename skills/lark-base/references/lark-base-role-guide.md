...
```javascript
lark_api({ tool: 'base', op: 'role-create', args: { base_token: '<base_token>', json: '{"role_name":"Reviewer","role_type":"custom_role","base_rule_map":{"copy":false,"download":false}}' } })
```

Rename a role while preserving its type:

```javascript
lark_api({ tool: 'base', op: 'role-update', args: { base_token: '<base_token>', role_id: '<role_id>', json: '{"role_name":"Finance Reviewer","role_type":"custom_role"}', yes: true } })
```

Grant read-only access to one table:

```javascript
lark_api({ tool: 'base', op: 'role-update', args: { base_token: '<base_token>', role_id: '<role_id>', json: '{"role_name":"Finance Reviewer","role_type":"custom_role","table_rule_map":{"Orders":{"perm":"read_only"}}}', yes: true } })
```
