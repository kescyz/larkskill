...
// 3️⃣ 再提交
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', json: '{"fields":{...}}' } })
```
...
```javascript
// 基本提交（填写普通字段）
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', json: '{"fields":{"服务评分":5,"评价内容":"服务态度好"}}' } })

// 带附件提交（需要额外提供 base_token）
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', base_token: '<base_token>', json: '{"fields":{"服务评分":5,"评价内容":"好"},"attachments":{"附件字段名":["./report.pdf","./photo.png"],"另一个附件字段":["./doc.docx"]}}' } })

// 使用应用身份（bot）
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', json: '{"fields":{...}}', as: 'bot' } })

// 预览 API 调用（不实际执行）
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: '<share_token>', json: '{"fields":{...}}', dry_run: true } })
```
...
```javascript
lark_api({ tool: 'base', op: 'form-submit', args: { share_token: 'shrbcvST8eZy0vk8zjVZ1CAXNye', json: '{"fields":{...}}' } })
```
