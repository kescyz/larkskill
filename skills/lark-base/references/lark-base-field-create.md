...
```javascript
lark_api({ tool: 'base', op: 'field-create', args: { base_token: '<base_token>', table_id: '<table_id>', json: '{"name":"预算","type":"number","style":{"type":"plain","precision":2}}' } })

lark_api({ tool: 'base', op: 'field-create', args: { base_token: '<base_token>', table_id: '<table_id>', json: '{"name":"状态","type":"select","multiple":false,"options":[{"name":"Todo","hue":"Blue","lightness":"Lighter"},{"name":"Done","hue":"Green","lightness":"Light"}]}' } })

lark_api({ tool: 'base', op: 'field-create', args: { base_token: '<base_token>', table_id: '<table_id>', json: '{"name":"负责人","type":"user","multiple":false,"description":"用于标记记录的直接负责人；协作约定可参考[团队字段约定](https://example.com/field-spec)"}' } })
```
