...
When creating or updating a lookup field via MCP tool `lark_api` with `"type": "lookup"`, read this guide first before constructing the field JSON body.

Do **not** construct the lookup field body before reading this guide.
...
1. Get all table names: call `lark_api` GET `/open-apis/base/v3/bases/{base_token}/tables` — returns `items[].table_name`
2. Get table structure: call `lark_api` GET `/open-apis/base/v3/bases/{base_token}/tables/{table_id}` — returns `fields[]`
...
```json
["project name", "==", { "type": "field_ref", "field": "project name" }]
```
...
|-----------|----------------------|---------|
| Text / Phone / Email / Url | String | `"Completed"` |
