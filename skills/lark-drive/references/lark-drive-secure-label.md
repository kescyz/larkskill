...
- `secure-label-list`：查询当前用户可用的密级标签，先拿到目标 `id`。
- `secure-label-update`：把目标云文档调整为指定密级标签。

这两个操作都使用用户身份（`as: 'user'`）。这两个 op 比当前 MCP catalog snapshot 更新，调用前先用 `lark_api_search` 发现其确切参数形态。修改密级前，通常先执行 `secure-label-list` 确认可用标签 ID。
