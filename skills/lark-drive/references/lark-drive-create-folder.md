...
> [!IMPORTANT]
> 如果文件夹是**以应用身份（bot）创建**的，如 `lark_api({ tool: 'drive', op: 'create-folder', args: { ..., as: 'bot' } })`，在创建成功后 MCP tool 会**尝试为当前用户自动授予该文件夹的 `full_access`（可管理权限）**。
>
...
- **原生 API 仍可用**：如果用户明确要求按底层 API 字段调用，仍可继续使用 `lark_api({ tool: 'drive', op: 'files.create_folder', args: { ... } })`
