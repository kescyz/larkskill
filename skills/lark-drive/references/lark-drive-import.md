...
> 只有导入完成并拿到新文档的 `token` / `url` 后，后续字段、记录、视图等表内操作才切换到 `lark_api({ tool: 'base', op: '...', args: { ... } })`。
...
// 预览底层调用链（上传 -> 创建任务 -> 轮询）
lark_api({ tool: 'drive', op: 'import', args: { file: './README.md', type: 'docx', dry_run: true } })
```
...
> [!IMPORTANT]
> 如果在线文档是**以应用身份（bot）导入创建**的，如 `lark_api({ tool: 'drive', op: 'import', args: { ..., as: 'bot' } })`，当某次结果**已经返回最终在线文档目标**后，MCP tool 会**尝试为当前用户自动授予该资源的 `full_access`（可管理权限）**。
>
> 这个自动授权有两种触发时机：
> - `drive +import` 的内置轮询窗口内已经完成，直接在 `+import` 中进行自动授权
> - `drive +import` 先返回 `ready=false` / `timed_out=true`，之后你再执行 `lark_api({ tool: 'drive', op: 'task_result', args: { scenario: 'import', ticket: '<TICKET>' } })`，当该查询第一次拿到最终在线文档目标时会自动授权
>
> 只有在已经拿到最终在线文档目标的那次结果里，才会返回 `permission_grant` 字段，明确说明授权结果：
> - `status = granted`：当前 CLI 用户已获得该导入结果的可管理权限
> - `status = skipped`：本地没有可用的当前用户 `open_id`，或当前结果还没有可授权目标，因此不会自动授权；可提示用户先完成登录（`lark_auth_login`），再让 AI / agent 继续使用应用身份（bot）授予当前用户权限
