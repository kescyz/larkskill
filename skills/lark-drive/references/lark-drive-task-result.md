...
// 2. 轮询导入结果
lark_api({ tool: 'drive', op: 'task_result', args: { scenario: 'import', ticket: '<IMPORT_TICKET>' } })
...
// 2. 继续查询导出结果
lark_api({ tool: 'drive', op: 'task_result', args: { scenario: 'export', ticket: '<EXPORT_TICKET>', file_token: '<SOURCE_DOC_TOKEN>' } })

// 3. 拿到 file_token 后下载
lark_api({ tool: 'drive', op: 'export-download', args: { file_token: '<EXPORTED_FILE_TOKEN>' } })
```
