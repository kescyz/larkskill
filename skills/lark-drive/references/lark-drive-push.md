...
lark_api({ tool: 'drive', op: 'push', args: {
  local_dir: './repo', folder_token: 'fldcnxxxxxxxxx',
  if_exists: 'smart'
} })
...
// （delete_remote 必须搭配 yes: true，否则会被 Validate 直接拒绝；
//   且 Validate 阶段会动态检查 space:document:delete scope，缺权限会立刻失败，
//   不会出现"上传成功了但是后面删除阶段挂了"的半同步状态）
lark_api({ tool: 'drive', op: 'push', args: {
  local_dir: './repo', folder_token: 'fldcnxxxxxxxxx',
  if_exists: 'overwrite', delete_remote: true, yes: true
} })
```
...
>
> 想一次性把权限补齐：使用 `lark_auth_login` 并申请 `drive:drive.metadata:readonly drive:file:upload space:folder:create space:document:delete` scope。
