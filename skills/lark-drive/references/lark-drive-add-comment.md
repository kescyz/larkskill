...
// 注意：MCP tool 会先查询 drive metas，只有白名单扩展名才允许评论
lark_api({ tool: 'drive', op: 'add-comment', args: {
...
lark_api_search({ query: 'drive file.comments create_v2' })

lark_api({ tool: 'drive', op: 'file.comments.create_v2', args: {
...
- 如果需要更底层的控制，仍可改用 `lark_api_search({ query: 'drive file.comments create_v2' })` + `lark_api({ tool: 'drive', op: 'file.comments.create_v2', args: { ... } })`。
