...
```js
// 默认：未指定位置时添加全文评论
lark_api({ tool: 'drive', op: 'add-comment', args: {
...
// 给 docx 文档的指定 block 添加局部评论（block_id 可通过 lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', detail: 'with-ids' } }) 获取）
lark_api({ tool: 'drive', op: 'add-comment', args: {
  doc: 'https://example.larksuite.com/docx/<DOC_ID>',
  block_id: '<BLOCK_ID>',
...
lark_api({ tool: 'drive', op: 'add-comment', args: {
  ...
  block_id: '<SLIDE_BLOCK_TYPE>!<XML_ELEMENT_ID>',
...
lark_api_search("drive file.comments create_v2")

lark_api({ tool: 'drive', op: 'file.comments', args: {
  method: 'create_v2',
  file_token: '<DOC_TOKEN>',
  file_type: 'docx',
  reply_elements: [{ type: 'text', text: '全文评论内容' }]
} })
```
...
| `block_id` | 局部评论时必填 | 目标块 ID，可通过 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', detail: 'with-ids' } })` 获取。**Sheet 评论**：格式为 `<sheetId>!<cell>`（如 `a281f9!D6`） |

## 行为说明

- **局部评论需要先获取 block ID**：先调用 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', doc: '<TOKEN>', detail: 'with-ids' } })` 获取带有 block ID 的文档内容，然后使用 `block_id` 指定目标块。
- **Review 场景优先局部评论**：审阅、校对、逐条指出问题时，必须先尝试定位到具体 block / 单元格 / slide 元素，并逐问题创建局部评论；不要把所有问题合并成一条全文评论。
- 未传 `block_id` 时，shortcut 默认创建**全文评论**；也可以显式传 `full_comment`。全文评论支持 `docx`、旧版 `doc` URL、白名单扩展名的 Drive file，以及最终可解析为 `doc`/`docx`/`file` 的 wiki URL。
...
- **Drive file 只支持全文评论**：file 目标不支持局部评论，不允许传 `block_id` 或 `selection_with_ellipsis`。由于当前 OpenAPI 要求 file 评论传入非空 `anchor.block_id`，CLI 会固定传占位值 `test`，UI 上仍表现为文件全文评论。
...
- **Slide 评论**：当 `doc` 为 slides URL、`type: 'slides'`，或 wiki 解析为 slides 时，必须传 `block_id: '<SLIDE_BLOCK_TYPE>!<XML_ELEMENT_ID>'`。CLI 会将其拆分映射到 `anchor.block_id` / `anchor.slide_block_type`。此时 `full_comment` 和 `selection_with_ellipsis` 不可用。
...
- 如果需要更底层的控制，仍可改用 `lark_api_search("drive file.comments create_v2")` + `lark_api({ tool: 'drive', op: 'file.comments', args: { method: 'create_v2' } })`。
