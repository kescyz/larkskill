...
// 原生命令（高级/分片上传）：预上传 + 完成上传（无 shortcut，走 raw API）
lark_api({ method: 'POST', path: '/open-apis/drive/v1/files/upload_prepare', body: {
...
lark_api({ method: 'POST', path: '/open-apis/drive/v1/files/upload_finish', body: {
  upload_id: '<UPLOAD_ID>', block_num: 1
} })

// 查看完整参数定义
lark_api_search({ query: 'drive files upload_prepare' })
```

> [!IMPORTANT]
> 如果文件是**以应用身份（bot）新建上传**的，如 `lark_api({ tool: 'drive', op: 'upload', args: { ..., as: 'bot' } })` 在上传成功后，MCP tool 会**尝试为当前用户自动授予该文件的 `full_access`（可管理权限）**。
>
...
> - `status = skipped`：本地没有可用的当前用户 `open_id`，因此不会自动授权；可提示用户先完成登录（`lark_auth_login`），再让 AI / agent 继续使用应用身份（bot）授予当前用户权限
