...
```js
// 上传到 Drive 文件夹
lark_api({ tool: 'drive', op: 'upload', args: { file: './report.pdf', folder_token: 'fldbc_xxx' } })

// 上传到 wiki 节点
lark_api({ tool: 'drive', op: 'upload', args: { file: './report.pdf', wiki_token: 'wikcn_xxx' } })

// 不指定目标时，上传到调用者的 Drive 根目录
lark_api({ tool: 'drive', op: 'upload', args: { file: './report.pdf' } })

// 自定义上传后的文件名
lark_api({ tool: 'drive', op: 'upload', args: { file: './report.pdf', name: '季度总结.pdf' } })

// 覆盖已存在文件（原地覆盖，保留 file_token）
lark_api({ tool: 'drive', op: 'upload', args: { file: './report.pdf', file_token: 'boxcn_existing_file' } })

// 原生调用（高级/分片上传）：预上传 + 完成上传
lark_api({ tool: 'drive', op: 'files.upload_prepare', args: {
...
lark_api({ tool: 'drive', op: 'files.upload_finish', args: {
  upload_id: '<UPLOAD_ID>',
  block_num: 1
} })

// 查看完整参数定义
lark_api_search("drive files upload_prepare")
```

> [!IMPORTANT]
> 如果文件是**以应用身份（bot）新建上传**的，如 `lark_api({ tool: 'drive', op: 'upload', args: { as: 'bot' } })` 在上传成功后，CLI 会**尝试为当前 CLI 用户自动授予该文件的 `full_access`（可管理权限）**。
>
> 如果这次调用传了 `file_token`，表示是在**覆盖已有文件**，CLI **不会**额外修改该文件权限。
>
...
- bot 模式下，`file_token` 覆盖只改文件内容；不会额外给当前 CLI 用户补 `full_access`
...
参数（预上传 `files.upload_prepare` args）：
