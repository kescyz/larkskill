...
处理文档评论 / 回复上的 reaction（点赞、表情、各表情数量、谁点了什么、添加/删除表情）。这个场景不常见，但规则比较集中：查询时只有在用户明确需要 reaction 信息时才带 `need_reaction=true`；写入时统一使用 `lark_api({ tool: 'drive', op: 'file.comment.reply.reactions', args: { method: 'update_reaction' } })`，操作对象始终是 `reply_id`。
...
- `file.comments` 的 `list` / `batch_query`、`file.comment.replys` 的 `list` 都支持通过指定`need_reaction`查询reaction信息。
- `need_reaction` 只在用户明确需要 reaction 信息时再带；如果用户只关心评论正文、回复正文、评论数 / 回复数，默认不要加。
- 遍历评论卡片并顺带拿 reaction：使用 `file.comments` 的 `list`。
- 已知评论 ID，批量查看 reaction：使用 `file.comments` 的 `batch_query`，并在请求体里带 `need_reaction=true`。
- 某张评论卡片下继续翻页拉 reply reaction：使用 `file.comment.replys` 的 `list`。
- 如果 `file.comments` 的 `list` 返回的某个 `item.has_more=true`，且用户要完整的 reply reaction 数据，后续每一页 `file.comment.replys` 的 `list` 都要持续带 `need_reaction=true`。
...
lark_api({ tool: 'drive', op: 'file.comment.replys', args: {
  method: 'list',
  file_token: '<DOC_TOKEN>',
...
- 添加 / 删除 reaction 时，使用 `lark_api({ tool: 'drive', op: 'file.comment.reply.reactions', args: { method: 'update_reaction' } })`。
...
lark_api({ tool: 'drive', op: 'file.comment.reply.reactions', args: {
  method: 'update_reaction',
  file_token: '<DOC_TOKEN>',
  file_type: 'docx',
  action: 'add',
  reply_id: '<REPLY_ID>',
