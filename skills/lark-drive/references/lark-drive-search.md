...
// 人类可读格式
lark_api({ tool: 'drive', op: 'search', args: { query: 'OKR', format: 'pretty' } })

// 翻页（format: 'json' 先拿 page_token）
lark_api({ tool: 'drive', op: 'search', args: { query: '方案', format: 'json' } })
...
| 参数 | 映射 | 说明 |
|---|---|---|
| `--mine` | `creator_ids = [当前用户 open_id]` | bool。一键”我 owner 的”（**不是**”我最初创建的”）；从当前登录用户身份解析 open_id，取不到直接报错（提示运行 `lark_auth_login`） |
