...
> 正确：`lark_api({ tool: 'drive', op: 'search', args: { query: '方案' } })`
> `search` 不接受位置参数；空 `query` 或省略 `query` 表示纯靠 filter 浏览（合法）。
>
> **列表型请求不要硬塞关键词**：如果用户只是要求"我这月创建的所有文档"、"最近半年我编辑过的文档"、"按类型分类统计"这类范围浏览 / 汇总请求，且没有给出标题片段或业务关键词，应使用 `query: ''` 搭配 `mine`、`created_*`、`edited_*`、`doc_types` 等过滤条件。不要把"查找"、"所有文档"、"最近更新过"、"按类型分类统计"这类动作词或统计意图放进 `query`，否则会把本来应靠 filter 命中的结果过度收窄。

### 自然语言 → 命令映射速查

| 用户说 | 命令 |
|---|---|
| 我这月创建的所有文档，按类型分类统计 | `lark_api({ tool: 'drive', op: 'search', args: { query: '', mine: true, created_since: '<YYYY-MM-DD>', created_until: '<YYYY-MM-DD>' } })` |
| 最近半年我编辑过的文档，看看哪些最近更新过 | `lark_api({ tool: 'drive', op: 'search', args: { query: '', edited_since: '6m', sort: 'edit_time' } })` |
| 最近一个月我编辑过的文档 | `lark_api({ tool: 'drive', op: 'search', args: { query: '', edited_since: '1m' } })` |
| 最近一个月我编辑过 且 我评论过的 | `lark_api({ tool: 'drive', op: 'search', args: { query: '', edited_since: '1m', commented_since: '1m' } })` |
| 最近一周我打开过的表格 | `lark_api({ tool: 'drive', op: 'search', args: { query: '', opened_since: '7d', doc_types: 'sheet' } })` |
| 我 owner 的所有文档（owner 语义，非"我最初创建"） | `lark_api({ tool: 'drive', op: 'search', args: { query: '', mine: true } })` |
| 我 owner、30-60 天前创建的文档（粗略"上个月"，按 30 天滑窗算；`mine` 是 owner，`created_*` 才是文档创建时间） | `lark_api({ tool: 'drive', op: 'search', args: { query: '', mine: true, created_since: '2m', created_until: '1m' } })` |
| 我 owner、2026 年 3 月创建的文档（精确日历月；同上，owner + 创建时间窗两个维度） | `lark_api({ tool: 'drive', op: 'search', args: { query: '', mine: true, created_since: '2026-03-01', created_until: '2026-04-01' } })` |
| 关键词"预算"，最近一周我打开过，按编辑时间降序 | `lark_api({ tool: 'drive', op: 'search', args: { query: '预算', opened_since: '7d', sort: 'edit_time' } })` |
| 某个 wiki space 下、我 owner 且 30-60 天前创建的 | `lark_api({ tool: 'drive', op: 'search', args: { query: '', mine: true, space_ids: 'space_xxx', created_since: '2m', created_until: '1m' } })` |
| 张三 owner / 负责的文档（注意是 owner 语义，不是张三最初创建的）| `lark_api({ tool: 'drive', op: 'search', args: { query: '', creator_ids: 'ou_zhangsan' } })` |
| 我最近 3 个月评论过的 docx | `lark_api({ tool: 'drive', op: 'search', args: { query: '', commented_since: '3m', doc_types: 'docx' } })` |

### 更多示例

```js
// 纯关键词搜索
lark_api({ tool: 'drive', op: 'search', args: { query: '季度总结' } })

// 使用服务端 query 高级语法
lark_api({ tool: 'drive', op: 'search', args: { query: 'intitle:方案' } })
lark_api({ tool: 'drive', op: 'search', args: { query: '"季度 总结"' } })
lark_api({ tool: 'drive', op: 'search', args: { query: '方案 OR 草稿' } })
lark_api({ tool: 'drive', op: 'search', args: { query: '方案 -草稿' } })

// 只搜某个文件夹下的文档
lark_api({ tool: 'drive', op: 'search', args: { query: '方案', folder_tokens: 'fld_123456' } })

// 只搜某个知识空间下的 Wiki
lark_api({ tool: 'drive', op: 'search', args: { query: '研发规范', space_ids: 'space_1234567890fedcba' } })

// 指定群内分享过的文档
lark_api({ tool: 'drive', op: 'search', args: { query: '方案', chat_ids: 'oc_1234567890abcdef' } })

// 只搜标题 / 只搜评论
lark_api({ tool: 'drive', op: 'search', args: { query: '周报', only_title: true } })
lark_api({ tool: 'drive', op: 'search', args: { query: '延期原因', only_comment: true } })

// 翻页（先拿 page_token）
lark_api({ tool: 'drive', op: 'search', args: { query: '方案' } })
lark_api({ tool: 'drive', op: 'search', args: { query: '方案', page_token: '<PAGE_TOKEN>' } })
```
...
| 参数 | 映射 | 说明 |
|---|---|---|
| `mine` | `creator_ids = [当前用户 open_id]` | bool。一键“我 owner 的”（**不是**“我最初创建的”）；从当前登录用户身份（`runtime.UserOpenId()`）解析 open_id，取不到直接报错（提示运行 `lark_auth_login`） |
| `creator_ids` (如 `ou_x,ou_y`) | `creator_ids = [...]` | 显式 open_id 列表，逗号分隔，按 **owner** 匹配；**与 `mine` 互斥** |
...
| 参数 | 映射 | 说明 |
|---|---|---|
| `doc_types` (如 `docx,sheet`) | `doc_types` | 逗号分隔。允许值：`doc,sheet,bitable,mindnote,file,wiki,docx,folder,catalog,slides,shortcut` |
| `folder_tokens` (如 `fld_a,fld_b`) | `folder_tokens`（仅 doc_filter） | 存在时只发 `doc_filter`；**与 `space_ids` 互斥** |
| `space_ids` (如 `sp_x`) | `space_ids`（仅 wiki_filter） | 存在时只发 `wiki_filter`；**与 `folder_tokens` 互斥** |
| `chat_ids` (如 `oc_x`) | `chat_ids` | 逗号分隔 |
| `sharer_ids` (如 `ou_x`) | `sharer_ids` | 逗号分隔，open_id |
...
> `sort`：CLI 只暴露服务端**正式支持**的 5 个值。服务端 enum 里 `CREATE_TIME_ASC` 协议标注"暂不支持"，`ENTITY_CREATE_TIME_ASC` / `ENTITY_CREATE_TIME_DESC` 已废弃，CLI 直接不放出来，传了会被 enum 校验拒掉。
...
- 返回 `{ total, has_more, page_token, results: [...] }`；所有 `*_time` 字段递归补 `*_time_iso`
...
- **时间表达**：
  ...
  - **日历表达**（"上个月"、"上周"、"本月"、"前年"、"今年 3 月"等明确日历单位）→ **必须算出绝对 `YYYY-MM-DD` 边界**（如"上个月" = 上一个日历月的 1 号 → 当月 1 号），**不要近似成 `1m`/`2m`**：这里 `m` 是固定 30 天、`y` 固定 365 天，跟日历差 0-3 天，月末月初尤其容易偏出去
...
- **原始返回**：用户要求"原始数据"、"接口返回"时，从返回的 JSON 直接取用，不做客户端精确过滤或摘要重写。
...
CLI 在发请求前会检查 `opened_since` 到有效 `opened_until`（没传则取 `now`）的跨度：
...
2. **先在当前 slice 内翻页**：返回的 `has_more = true` 且用户想看更多时，把 `opened_since` / `opened_until` 改成 notice 里 `[slice 1/N current]` 行给出的**具体时间值**（**不要继续用原始的 `opened_since: '1y'` 这种相对值**——CLI 每次调用都按 `time.Now()` 重算窗口，相对值 + `page_token` 一起跑会让 page_token 绑到一个漂移的窗口上、结果静默失真），加 `page_token` 继续翻，直到 `has_more = false`
3. **再切换到下一个 slice**：当前 slice 翻完后，如果用户还要"更老的"，用 notice 里列的 slice 2 的 `opened_since` / `opened_until` 值，**其他参数（`query`、`doc_types`、`sort`……）保持原样，`page_token` 不带**，重新发请求
