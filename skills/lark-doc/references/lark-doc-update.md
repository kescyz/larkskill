...
>
> **Markdown 局限 & block ID 前提：** Markdown 不携带 block ID，也无样式（颜色、对齐、callout 等）。需要按 block ID 定位（`block_*` 指令的 `block_id`）时，先 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', detail: 'with-ids' } })` **配合 `scope`（`outline` / `range` / `keyword` / `section`）局部获取**目标段落，不要全量 fetch。拿到 block ID 后 `content` 仍可用 Markdown，只是写入内容不带样式。
...
| `append` | 在文档末尾追加内容（等价于 `block_insert_after`，`block_id: -1`） | `content` |
...
```js
// 简单文本替换
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', pattern: '张三', content: '李四' } })

// 替换为富文本（加粗 + 链接）
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', pattern: '旧链接', content: '<b>新链接</b> <a href="https://example.com">点击查看</a>' } })

// 仅当用户明确要求时才使用 Markdown
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', doc_format: 'markdown', pattern: '旧内容', content: '新内容' } })

// Markdown 模式下支持跨行匹配（pattern 与 content 都需要真实换行）
// 多行内容推荐 content: '@file.md'，避免转义踩坑
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', doc_format: 'markdown', pattern: '## 旧标题\n\n第一段原文\n\n第二段原文', content: '## 新标题\n\n改写后的第一段\n\n改写后的第二段' } })

// Markdown 模式下使用 `前缀...后缀` 省略号匹配首尾特征明显的大段内容
// 下例会把「## 旧标题」到「结束语。」之间的所有内容整体替换
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', doc_format: 'markdown', pattern: '## 旧标题...结束语。', content: '## 新标题\n\n重写后的正文...\n\n新的结束语。' } })

// 删除文本：content 传空字符串即可
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', pattern: '废弃的内容', content: '' } })
```

### block_insert_after — 在指定 block 之后插入

```js
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_insert_after', block_id: '目标 block_id', content: '<h2>新章节</h2><ul><li>要点 1</li><li>要点 2</li></ul>' } })
```

### block_replace — 替换指定 block

```js
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_replace', block_id: '目标 block_id', content: '<p>替换后的段落内容</p>' } })
```

### block_delete — 删除指定 block

```js
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_delete', block_id: '目标 block_id' } })
```

### overwrite — 全文覆盖

```js
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'overwrite', content: '<title>全新文档</title><h1>概述</h1><p>新的内容</p>' } })
```
...
```js
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'append', content: '<h2>新增章节</h2><p>追加的内容</p>' } })
```

> 等价于 `block_insert_after`（`block_id: -1`），无需先获取 block ID。
...
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_copy_insert_after', block_id: '锚点 block_id', src_block_ids: 'block_a,block_b,block_c' } })
```
...
```js
// 移动到页面末尾
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_move_after', block_id: '-1表示末尾，page_id表示开头，blk', src_block_ids: 'block_a,block_b' } })
```
...
1. **获取文档内容和 block ID**：
   ```js
   lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', doc: '<doc_id>', detail: 'with-ids' } })
   ```
...
3. **执行更新**：
   ```js
   // 替换特定 block
   lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_replace', block_id: 'blkcnXXXX', content: '<p>新内容</p>' } })

   // 在某 block 后插入
   lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_insert_after', block_id: 'blkcnXXXX', content: '<h2>追加的章节</h2>' } })
   ```
...
```js
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'str_replace', pattern: 'v1.0', content: 'v2.0' } })
```

## 画板处理

> **`lark_api({ tool: 'docs', op: 'update' })` 不能直接编辑已有画板的内容。** 本命令只能**新增**画板块；要修改已有画板，先用 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2' } })` 取到 `<whiteboard token="...">`，再按 [`lark-doc-whiteboard.md`](lark-doc-whiteboard.md) 启动 SubAgent 读取 [`lark-whiteboard`](../../lark-whiteboard/SKILL.md) 并写入。
