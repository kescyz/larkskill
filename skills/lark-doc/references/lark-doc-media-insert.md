...
`from_clipboard: true` 失败（剪切板里不是图片 / 没有图片 / Linux 上三个工具都没装）时，命令会返回 `clipboard contains no image data`（或类似的平台错误）。**这不是错误退出理由，而是 fallback 信号。**
...
```js
// 🟢 推荐：从剪切板直接插入（无需先存盘）
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnXXX', from_clipboard: true } })

// 从本地文件插入
// 除了上传本地文件，还可以在 docs update 时直接通过网络 URL 插入图片，无需先下载到本地：
lark_api({ tool: 'docs', op: 'update', args: { api_version: 'v2', doc: '<doc_id>', command: 'block_insert_after', block_id: '目标 block_id', content: '<img href="https://example.com/photo.png"/>' } })

// 插入图片（默认）
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnXXX', file: './image.png' } })

// doc 支持直接传 docx URL（自动提取 document_id）
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'https://xxx.feishu.cn/docx/doxcnXXX', from_clipboard: true } })

// 如果上一步是 create-doc，优先传返回值里的 doc_id
// 不要把 /wiki/... 形式的 doc_url 直接传给 docs media-insert
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnReturnedByCreateDoc', file: './image.png' } })

// 插入文件（非图片）
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnXXX', file: './spec.pdf', type: 'file' } })

// 图片对齐与描述（caption）
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnXXX', from_clipboard: true, align: 'center', caption: '架构图' } })

// Insert image with explicit display width (height auto-computed from aspect ratio)
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnXXX', file: './banner.png', width: 800, align: 'center' } })

// Insert image with explicit width and height
lark_api({ tool: 'docs', op: 'media-insert', args: { doc: 'doxcnXXX', from_clipboard: true, width: 800, height: 447, caption: 'architecture diagram' } })
```
...
> [!IMPORTANT]
> 如果上一步是 [`lark-doc-create`](lark-doc-create.md)，并且它在知识库/知识空间场景下返回的是 `/wiki/...` 形式的 `doc_url`，后续调用 `lark_api({ tool: 'docs', op: 'media-insert' })` 时应优先传 `doc_id`，不要直接传这个 `doc_url`。
