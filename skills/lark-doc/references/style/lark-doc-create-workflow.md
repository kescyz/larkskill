...
3. `lark_api({ tool: 'docs', op: 'create', args: { api_version: 'v2' } })` **只建骨架**：标题 + 开头 `<callout>` + 各级标题 + 每节一句占位摘要
   - ⚠️ **不要**一次性把完整章节内容塞进 `content`。超长 `content` 容易触发字符/参数限制。
   - 完整内容留到第二波，由各 Agent 用 `lark_api({ tool: 'docs', op: 'update', args: { command: 'append' } })` 或 `block_insert_after` 分段写入。
...
4. Spawn Agent 并行撰写各章节。每个 Agent 需收到：
   ...
   - 使用 `lark_api({ tool: 'docs', op: 'update', args: { command: 'append' } })` 或 `block_insert_after` 写入

### 第三波 — 整合审查 + 画板意图识别（串行）

5. `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', detail: 'with-ids' } })` 获取文档，审查整体效果
...
9. Spawn 内容改写 Agent 定向润色：
   ...
   - 主要章节间补充 `<hr/>`
   - 本地图片使用 `lark_api({ tool: 'docs', op: 'media-insert' })` 插入
