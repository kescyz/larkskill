...
2. **Execute（执行）** — 调用相应的 `lark_api({ tool: 'docs' })` 工具，或 **spawn** Agent 子任务并行推进
...
1. **选择读取范围**（节省上下文的关键）：
   - 用户只改某一节 / 文档较大 → 先 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', scope: 'outline', max_depth: 2 } })` 拿目录，再 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', scope: 'section', start_block_id: '<目标标题id>', detail: 'with-ids' } })` 精读该节（`section` 会自动展开到下一个同级/更高级标题前，不用手动算结束 block id）
   - 需要精确跨节区间 → `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', scope: 'range', start_block_id: 'xxx', end_block_id: 'yyy' } })`（或 `end_block_id: -1` 读到末尾）
   - 用户只给了模糊关键词 → `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', scope: 'keyword', keyword: 'xxx', context_before: 1, context_after: 1, detail: 'with-ids' } })`
   - 用户明确要改整篇 → `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', detail: 'with-ids' } })`
...
内容改写 Agent 必须收到：文档 token、章节范围（标题/block ID）、`lark-doc-xml.md` 和 `lark-doc-style.md` 路径、具体的 `lark_api({ tool: 'docs', op: 'update' })` command 和 `block_id`。
...
**上下文节省提示**：Agent 如需在自己负责的章节内重新读取内容，优先用 `lark_api({ tool: 'docs', op: 'fetch', args: { api_version: 'v2', scope: 'section', start_block_id: '<章节标题id>' } })`（自动覆盖整节），或 `scope: 'range'` 配合 `start_block_id: 'xxx'`, `end_block_id: 'yyy'` 精确区间，只拉自己的章节，不要重复拉全文。
