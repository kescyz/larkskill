...
> **默认草稿**：`+forward` 默认保存为草稿，不会立即发送。如需立即发送，添加 `confirm_send: true` 参数（仅在用户明确确认后使用）。

本 skill 对应 shortcut：`lark_api({ tool: 'mail', op: 'forward' })`。
...
**方式 A（推荐）** — 创建转发草稿（不带 `confirm_send: true`）：
```js
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: '<收件人>' } })
```
→ 返回 `draft_id`
...
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'alice@example.com', body: '<p>FYI，请看下面原邮件。</p>' } })

// 转发并附加说明 + 抄送（草稿）
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'alice@example.com', cc: 'bob@example.com', body: '<b>请参考</b>' } })

// 转发时插入内嵌图片（推荐：直接用相对路径，自动解析）
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'alice@example.com', body: '<p>详见图示：<img src="./logo.png" /></p>' } })

// 纯文本转发（仅在内容极简时使用）
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'alice@example.com' } })

// 确认发送（用户明确确认后才可使用）
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'alice@example.com', confirm_send: true } })
```
...
| `to` | 是 | 收件人邮箱，多个用逗号分隔 |
| `body` | 否 | 转发时附加的说明文字。推荐使用 HTML 获得富文本排版；也支持纯文本。根据转发正文和原邮件正文自动检测 HTML。使用 `--plain-text` 可强制纯文本模式。支持 `<img src="./local.png" />` 相对路径自动解析为内嵌图片（仅支持相对路径，不支持绝对路径）。与 `--body-file` 互斥 |
...
| `signature_id` | 否 | 签名 ID。附加邮箱签名到转发正文与引用块之间。运行 `mail +signature` 查看可用签名。不可与 `--plain-text` 同时使用 |
...
```js
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'bob@example.com', body: '<p>FYI</p>' } })
```
...
```js
// 方式 A: 创建转发草稿
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'bob@example.com', body: '<p>FYI，请查收。</p>' } })
// → 返回 draft_id
...
// 方式 B: 用户已明确确认时，直接发送
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<邮件ID>', to: 'bob@example.com', body: '<p>FYI，请查收。</p>', confirm_send: true } })
```
...
`+forward` 操作的是单封邮件（`message_id`），但转发整个会话时应 forward **会话中最后一条消息**，因为邮件客户端会将完整的回复链嵌套在最新一条中。典型流程：
...
// 3. 转发该消息
lark_api({ tool: 'mail', op: 'forward', args: { message_id: '<最后一条的message_id>', to: 'recipient@example.com', body: '请过目' } })
```
...
`+forward` 创建的草稿正文包含引用区（原邮件的引用块）。如果需要编辑转发草稿的正文，**必须通过 `patch_file` 使用 `set_reply_body` op**，它仅替换用户撰写部分，自动保留引用区。value 只传新的用户撰写内容，不要包含引用区。

```js
// 编辑转发草稿正文（自动保留引用区）
lark_api({ tool: 'mail', op: 'draft-edit', args: { draft_id: '<draft_id>', patch: { ops: [{ op: 'set_reply_body', value: '<p>修改后的转发附言</p>' }] } } })
```
...
- `lark_api({ tool: 'mail', op: 'send' })` — 发送新邮件
- `lark_api({ tool: 'mail', op: 'reply' })` — 回复邮件
- `lark_api({ tool: 'mail', op: 'user_mailbox.messages.get' })` — 查看邮件详情
