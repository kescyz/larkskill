...
如需修改已有草稿，不要使用此命令，请使用 `lark_api({ tool: 'mail', op: 'draft-edit' })`。
...
```js
// 创建 HTML 草稿（推荐）
lark_api({ tool: 'mail', op: 'draft-create', args: { to: 'alice@example.com', subject: '周报',
  body: '<p>本周进展：</p><ul><li>完成 A 模块</li></ul>' } })

// 不带收件人的 HTML 草稿（用户之后可自行添加）
lark_api({ tool: 'mail', op: 'draft-create', args: { subject: '周报', body: '<p>草稿内容</p>' } })

// 带附件和内嵌图片的 HTML 草稿（推荐：直接用相对路径，自动解析）
lark_api({ tool: 'mail', op: 'draft-create', args: { to: 'alice@example.com', subject: '预览图', body: '<p>见附件和图：<img src="./logo.png" /></p>', attach: './report.pdf' } })

// 纯文本草稿（仅在内容极简时使用）
lark_api({ tool: 'mail', op: 'draft-create', args: { to: 'alice@example.com', subject: '简短通知', body: '收到，谢谢' } })
```
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `to` | 否 | 完整收件人列表，多个用逗号分隔。支持 `Alice <alice@example.com>` 格式。省略时草稿不带收件人（之后可通过 `+draft-edit` 添加） |
| `subject` | 是 | 草稿主题 |
...
| `cc` | 否 | 完整抄送列表，多个用逗号分隔 |
| `bcc` | 否 | 完整密送列表，多个用逗号分隔。与 `--event-*` 不兼容（见 `+send` 日程邀请约束） |
...
```js
// 1. 创建草稿
lark_api({ tool: 'mail', op: 'draft-create', args: { to: 'alice@example.com', subject: 'Q1 报告', body: '请查收附件中的报告。', attach: './q1-report.pdf' } })
...
lark_api({ tool: 'mail', op: 'draft-create', args: {
...
- `lark_api({ tool: 'mail', op: 'draft-edit' })` — 编辑已有草稿
- `lark_api({ tool: 'mail', op: 'user_mailbox.drafts.send' })` — 发送已有草稿
- `lark_api({ tool: 'mail', op: 'user_mailbox.drafts.get' })` — 获取草稿内容
- `lark_api({ tool: 'mail', op: 'reply' })` / `reply-all` / `forward` — 创建回复/转发草稿（默认），或加 `confirm_send: true` 发送
