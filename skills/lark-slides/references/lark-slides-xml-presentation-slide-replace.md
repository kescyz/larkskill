...
```javascript
lark_api({ tool: 'slides', op: 'xml_presentation.slide.replace', args: { xml_presentation_id: '<xml_presentation_id>', slide_id: '<slide_id>', parts: [...], as: 'user' } })
```
...
```javascript
lark_api({ tool: 'slides', op: 'xml_presentation.slide.replace', args: { xml_presentation_id: 'slides_example_presentation_id', slide_id: 'slide_example_id', parts: [{ action: 'block_replace', block_id: 'bab', replacement: '<shape type="text" topLeftX="80" topLeftY="80" width="800" height="120"><content textType="title"><p>新标题</p></content></shape>' }], as: 'user' } })
```
...
lark_api({ tool: 'slides', op: 'xml_presentation.slide.replace', args: { xml_presentation_id: PID, slide_id: SID, parts: [{ action: 'block_insert', insertion: `<img src="${TOKEN}" topLeftX="500" topLeftY="100" width="200" height="150"/>` }], as: 'user' } })
```

### 多条 parts 原子执行

```javascript
lark_api({ tool: 'slides', op: 'xml_presentation.slide.replace', args: { xml_presentation_id: 'slides_example_presentation_id', slide_id: 'slide_example_id', parts: [{ action: 'block_replace', block_id: 'bab', replacement: '<shape type="text" topLeftX="80" topLeftY="80" width="800" height="120"><content textType="title"><p>新标题</p></content></shape>' }, { action: 'block_insert', insertion: '<img src="<file_token>" topLeftX="700" topLeftY="400" width="180" height="100"/>' }], as: 'user' } })
```
...
7. **执行前必做**：`lark_api_search({ query: 'slides xml_presentation.slide.replace' })` 查看最新参数结构。
