...
```js
lark_api({ tool: 'slides', op: 'xml_presentation.slide.replace', args: { /* ...params + data 字段 */ }, as: 'user' })
```

## 参数说明

`args` 同时包含路径/查询参数（`xml_presentation_id`、`slide_id`、`revision_id`、`tid`）和请求体字段（`parts`）。
...
lark_api({
  tool: 'slides',
  op: 'xml_presentation.slide.replace',
  args: {
    xml_presentation_id: 'slides_example_presentation_id',
    slide_id: 'slide_example_id',
    parts: [
...
lark_api({
  ...
  args: {
    ...
    parts: [
      {
        action: 'block_insert',
        insertion: '<img src="FILE_TOKEN" topLeftX="500" topLeftY="100" width="200" height="150"/>'
      }
    ]
  },
  as: 'user'
...
- [replace-slide](lark-slides-replace-slide.md) — 块级替换 shortcut（推荐，自动注入 id）
