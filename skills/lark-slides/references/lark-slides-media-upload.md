...
lark_api({ tool: 'slides', op: 'media-upload', args: { file: './pic.png', xml_presentation_id: 'slidesXXXXXXXXXXXXXXXXXXXXXX', as: 'user' } })

// 传 slides URL 也行
lark_api({ tool: 'slides', op: 'media-upload', args: { file: './chart.png', xml_presentation_id: 'https://xxx.feishu.cn/slides/slidesXXXXXXXXXXXXXXXXXXXXXX', as: 'user' } })

// 传 wiki URL（auto-resolves wiki.spaces.get_node，校验 obj_type=slides）
lark_api({ tool: 'slides', op: 'media-upload', args: { file: './pic.png', xml_presentation_id: 'https://xxx.feishu.cn/wiki/wikcnXXXXXX', as: 'user' } })
```
...
lark_api({ tool: 'slides', op: 'xml_presentation.slide.create', args: { xml_presentation_id: PRES_ID, slide: { content: `<slide xmlns="http://www.larkoffice.com/sml/2.0"><data><img src="${TOKEN}" topLeftX="100" topLeftY="100" width="320" height="180"/></data></slide>` }, as: 'user' } })
```
...
lark_api({ tool: 'slides', op: 'create', args: { title: '图测试', slides: ['<slide xmlns="http://www.larkoffice.com/sml/2.0"><data><img src="@./pic.png" topLeftX="100" topLeftY="100" width="320" height="180"/></data></slide>'], as: 'user' } })
```
...
lark_api({ tool: 'slides', op: 'replace-slide', args: { xml_presentation_id: PRES_ID, slide_id: SID, parts: [{ action: 'block_insert', insertion: `<img src="${TOKEN}" topLeftX="500" topLeftY="100" width="200" height="150"/>` }], as: 'user' } })
```
...
| 1061004 | forbidden：当前身份对该演示文稿无编辑权限 | 确认当前身份（user 或 bot）对目标 PPT 有编辑权限。bot 模式常见原因：PPT 不是该 bot 创建的——可用 `op: 'create', args: { as: 'bot' }` 新建，或以 user 身份给 bot 授权 `lark_api({ tool: 'drive', op: 'permission.members.create', args: { as: 'user', ... } })` |
