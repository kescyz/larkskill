...
lark_api({
  tool: 'slides',
  op: 'media-upload',
  args: { file: './pic.png', presentation: 'slidesXXXXXXXXXXXXXXXXXXXXXX' },
  as: 'user'
})
...
lark_api({
  tool: 'slides',
  op: 'media-upload',
  args: { file: './chart.png', presentation: 'https://xxx.feishu.cn/slides/slidesXXXXXXXXXXXXXXXXXXXXXX' },
  as: 'user'
})

// 传 wiki URL（自动 wiki.spaces.get_node 解析为真实 token，校验 obj_type=slides）
lark_api({
  tool: 'slides',
  op: 'media-upload',
  args: { file: './pic.png', presentation: 'https://xxx.feishu.cn/wiki/wikcnXXXXXX' },
  as: 'user'
})
```
...
> [!IMPORTANT]
> **路径必须在 CWD 内**：`file: '/abs/path/x.png'` 或 `file: '../up/x.png'` 会被拒绝（报 `unsafe file path`）。如果素材在别的目录，先 `cd` 过去再执行。
...
lark_api({
  ...
  args: {
    ...
    slide: { content: '<slide xmlns="http://www.larkoffice.com/sml/2.0"><data><img src="FILE_TOKEN" topLeftX="100" topLeftY="100" width="320" height="180"/></data></slide>' }
  },
  as: 'user'
})
```

### 新建带图 PPT（推荐用 `create` 的 `slides` 的 `@` 占位符，一步到位）
...
| 1061004 | forbidden：当前身份对该演示文稿无编辑权限 | 确认当前身份（user 或 bot）对目标 PPT 有编辑权限。bot 模式常见原因：PPT 不是该 bot 创建的——可用 `lark_api({ tool: 'slides', op: 'create', as: 'bot' })` 新建，或以 user 身份给 bot 授权 `lark_api({ tool: 'drive', op: 'permission.members.create', as: 'user' })` |
