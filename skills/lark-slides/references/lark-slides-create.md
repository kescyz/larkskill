...
- **`permission_grant`**（object，可选）：仅 `as: 'bot'` 时返回，说明是否已自动为当前用户授予可管理权限

> [!IMPORTANT]
> 不传 `slides` 时，`lark_api({ tool: 'slides', op: 'create' })` 只创建空白演示文稿。创建后需要使用 `lark_api({ tool: 'slides', op: 'xml_presentation.slide.create' })` 逐页添加 slide 内容。
>
> 传了 `slides` 时，先创建空白演示文稿，再逐页调用 `xml_presentation.slide.create` 添加页面。如果某一页添加失败，会停止并报错，已创建的演示文稿和已添加的页面会保留。
>
> 如果演示文稿是**以应用身份（bot）创建**的，如 `lark_api({ tool: 'slides', op: 'create', as: 'bot' })`，会**尝试为当前用户自动授予该演示文稿的 `full_access`（可管理权限）**。
>
...
JSON string 数组，每个元素是一页 slide 的完整 XML。内部负责包装成 API 所需的 `{"slide": {"content": "..."}}` 格式并逐页调用。
...
- `src` 不以 `@` 开头的会原样保留，但**只允许写 `lark_api({ tool: 'slides', op: 'media-upload' })` 拿到的 `file_token`**；**禁止写 http(s) 外链 URL**：飞书 slides 渲染端不会代理外链图片，外链 src 通常显示破图。要用网图必须先下载到 CWD 内、再走上传流程
...
`create` 的 `slides` 数组只在新建 PPT 时使用 `@` 占位符。给已有 PPT 加带图新页要分两步：
...
lark_api({
  tool: 'slides',
  op: 'media-upload',
  args: { file: './pic.png', presentation: 'PRES_ID' },
  as: 'user'
})
...
lark_api({
  ...
  args: {
    ...
    slide: { content: '<slide xmlns="http://www.larkoffice.com/sml/2.0"><data><img src="FILE_TOKEN" topLeftX="100" topLeftY="100" width="200" height="200"/></data></slide>' }
  },
  as: 'user'
})
```
...
如果没有使用 `slides`，`create` 返回的 `xml_presentation_id` 用于后续操作：

```js
// 第 1 步：创建空白 PPT，记录返回的 xml_presentation_id（PRES_ID）
lark_api({ tool: 'slides', op: 'create', args: { title: '项目汇报' } })
...
lark_api({
  ...
  args: {
    xml_presentation_id: 'PRES_ID',
    slide: { content: '<slide xmlns="http://www.larkoffice.com/sml/2.0">...</slide>' }
  },
  as: 'user'
})
```
