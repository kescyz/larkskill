...
```js
// 读时拿当前 revision_id（在返回的 data.revision_id 字段中）
lark_api({
  tool: 'slides',
  op: 'xml_presentation.slide.get',
  args: { xml_presentation_id: 'PID', slide_id: 'SID' },
...
lark_api({
  ...
  args: {
    presentation: 'PID',
    slide_id: 'SID',
    parts: [{ action: 'block_insert', insertion: '<img src="FILE_TOKEN" topLeftX="500" topLeftY="100" width="200" height="150"/>', insert_before_block_id: 'baa' }]
  },
  as: 'user'
})
```
...
> **`<img>` 必须用 `file_token`**，不能用外链 URL——先 `lark_api({ tool: 'slides', op: 'media-upload', args: { file: './pic.png', presentation: 'PID' } })` 拿 token。
