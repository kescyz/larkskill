...
```javascript
const PID = 'xml_presentation_id_here';
const SID = 'slide_id_here';
...
// 写时传该版本号，服务端以此为 base
lark_api({ tool: 'slides', op: 'replace-slide', args: { xml_presentation_id: PID, slide_id: SID, revision_id: REV, parts: [{ action: 'block_replace', block_id: 'bUn', replacement: '<shape type="rect" topLeftX="100" topLeftY="100" width="200" height="100"/>' }], as: 'user' } })
```
...
```javascript
lark_api({ tool: 'slides', op: 'replace-slide', args: { xml_presentation_id: PID, slide_id: SID, parts: [{ action: 'block_replace', block_id: 'bab', replacement: '<shape type="text" topLeftX="80" topLeftY="80" width="800" height="120"><content textType="title"><p>新标题</p></content></shape>' }], as: 'user' } })
```
...
```javascript
lark_api({ tool: 'slides', op: 'replace-slide', args: { xml_presentation_id: PID, slide_id: SID, parts: [{ action: 'block_insert', insertion: `<img src="${FILE_TOKEN}" topLeftX="500" topLeftY="100" width="200" height="150"/>`, insert_before_block_id: 'baa' }], as: 'user' } })
```
...
```javascript
// 从文件或其他来源读取 parts 数组后传入
lark_api({ tool: 'slides', op: 'replace-slide', args: { xml_presentation_id: PID, slide_id: SID, parts: /* parsed JSON array from parts.json */, as: 'user' } })
```
