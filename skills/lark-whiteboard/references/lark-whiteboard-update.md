...
```
lark_api({ tool: 'whiteboard', op: 'update', args: { whiteboard_token: '<画板Token>', input_format: 'plantuml', source: '<contents of diagram.puml>', overwrite: true, as: 'user' } })
```
...
```
lark_api({ tool: 'whiteboard', op: 'update', args: { whiteboard_token: '<画板Token>', input_format: 'mermaid', source: '@./diagram.mmd', overwrite: true, as: 'user' } })
```
...
```bash
# 使用 whiteboard-cli 生成 OpenAPI 格式并保存到文件
npx -y @larksuite/whiteboard-cli@^0.2.11 -i <产物文件> --to openapi --format json -o ./temp.json
```

```
lark_api({ tool: 'whiteboard', op: 'update', args: { whiteboard_token: '<画板Token>', source: '<contents of ./temp.json>', input_format: 'raw', idempotent_token: '<10+字符唯一串>', as: 'user' } })
```
...
```
lark_api({ tool: 'whiteboard', op: 'update', args: { whiteboard_token: '<画板Token>', idempotent_token: '<10+字符唯一串>', input_format: 'raw', source: '@./temp.json', overwrite: true, as: 'user' } })
```
