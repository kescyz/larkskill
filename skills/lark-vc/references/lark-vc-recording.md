...
本 skill 对应 shortcut：`lark_api({ tool: 'vc', op: '+recording', args: {...} })`。

## 命令

```javascript
// 通过会议 ID 查询（逗号分隔支持批量，最多 50 个）
lark_api({ tool: 'vc', op: '+recording', args: { 'meeting-ids': '69xxxxxxxxxxxxx28' } })
lark_api({ tool: 'vc', op: '+recording', args: { 'meeting-ids': '69xxxxxxxxxxxxx28,69xxxxxxxxxxxxx29' } })

// 通过日程事件 ID 查询
lark_api({ tool: 'vc', op: '+recording', args: { 'calendar-event-ids': 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_0' } })

// 输出格式
lark_api({ tool: 'vc', op: '+recording', args: { 'meeting-ids': '69xxxxxxxxxxxxx28', format: 'json' } })

// 预览 API 调用
lark_api({ tool: 'vc', op: '+recording', args: { 'meeting-ids': '69xxxxxxxxxxxxx28', 'dry-run': true } })
```
...
| 输入参数 | 获取方式 |
|---------|---------|
| `meeting_id` | 使用 `lark_api({ tool: 'vc', op: '+search', args: {...} })` 搜索历史会议，取结果中的 `id` 字段 |
| `calendar_event_id` | 使用 `lark_api({ tool: 'calendar', op: '+agenda', args: {...} })` 查看日程，取结果中的 `event_id` 字段 |
...
lark_api({ tool: 'vc', op: '+recording', args: { 'meeting-ids': 'xxx' } })

// 第 2 步：使用上一步返回的 minute_token 下载妙记文件
lark_api({ tool: 'minutes', op: '+download', args: { 'minute-tokens': '<minute_token>' } })
```
...
lark_api({ tool: 'minutes', op: 'minutes get', args: { params: { minute_token: '<minute_token>' } } })
```
...
lark_api({ tool: 'vc', op: '+search', args: { query: '周会', start: '2026-03-10' } })

// 第 2 步：使用上一步返回的 meeting_ids 查询录制，拿到 minute_tokens
lark_api({ tool: 'vc', op: '+recording', args: { 'meeting-ids': '<ids>' } })

// 第 3 步：使用其中一个 minute_token 下载妙记文件
lark_api({ tool: 'minutes', op: '+download', args: { 'minute-tokens': '<token>' } })
```
...
lark_api({ tool: 'vc', op: '+recording', args: { 'calendar-event-ids': '<event_id>' } })
