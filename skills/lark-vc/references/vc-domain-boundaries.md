...
```javascript
lark_api({ tool: 'vc', op: '+search', args: { start: '<YYYY-MM-DD>', end: '<YYYY-MM-DD>', format: 'json' } })
```
...
```javascript
lark_api({ tool: 'vc', op: '+notes', args: { 'meeting-ids': '<meeting_id1>,<meeting_id2>' } })
```
...
```javascript
lark_api({ tool: 'vc', op: '+notes', args: { 'minute-tokens': '<minute_token1>,<minute_token2>' } })
```
