...
name: lark-vc
version: 2.0.0
description: "Lark Video Conferencing: search ended meetings, query note artifacts (summaries, todos, chapters, transcripts, recommended keywords), and participant snapshots via LarkSkill MCP. Use for historical meetings; use lark-calendar for future schedules. Use lark-vc-agent for real-time in-meeting events and Agent join/leave."
metadata:
  requires:
    mcp: "larkskill"
...
- **MainDoc**: The main document of AI-generated meeting notes, containing the AI-generated summary and todos. Corresponds to `note_doc_token`.
...
2. Only filters such as keyword, time range, participant, organizer, and meeting room are supported. If a filter is unsupported, inform the user.
3. When search results contain multiple records, be sure to handle paginated data retrieval — do not miss any meeting records.
...
2. Only read document content to retrieve the summary, todos, chapter, or keyword artifacts when the user explicitly requests them.
3. When reading AI-generated notes (`note_doc_token`), the **first `<whiteboard>`** tag in the note document is the cover image (AI-generated summary visualization) — download and display it to the user at the same time:
```javascript
// 1. Read note content
...
// 3. Download the cover image to the aggregation directory (same directory as the transcript and recording, keeping artifacts together)
...
```
> **Artifact directory convention**: All downloaded artifacts for the same meeting (recording, transcript, cover image, etc.) go under `./minutes/{minute_token}/`. This matches the default output location of `minutes download` and `vc notes`, making it easy for the Agent to aggregate. Explicit paths (e.g. cover image) must be manually aligned to the same directory.

> **Note-related documents — choose based on user intent:**
> - `note_doc_token` → **AI-generated meeting notes** (AI summary + todos + chapters)
> - `meeting_notes` → **User-bound meeting notes** (a document the user manually linked to the meeting; only returned via the `calendar_event_ids` path)
> - `verbatim_doc_token` → **Verbatim transcript** (complete sentence-by-sentence record with speaker and timestamp) — use this when the user says "verbatim transcript", "full record", or "who said what"
> - **Minutes keywords** → recommended keywords extracted from the meeting, returned by `vc-notes` alongside the other artifacts. Use these when the user asks for meeting keywords, key topics, or tags. See [references/lark-vc-notes.md](references/lark-vc-notes.md) for the keyword field shape.
> - When the user says "notes", "summary", or "note content", return both `note_doc_token` and `meeting_notes` (if available)
...
2. When you only need basic information such as document name and URL:
   ...
   lark_api({ tool: 'drive', op: 'metas batch_query', args: { request_docs: [{ doc_token: '<token1>', doc_type: 'docx' }, { doc_token: '<token2>', doc_type: 'sheet' }], with_url: true } })
...
When the user asks "who attended this meeting", "what participants were in this meeting", or "did so-and-so attend" — these are **participant snapshot** questions. Use **`vc meeting get` with `with_participants: true`**: this is the server-side participant snapshot API, does not depend on bot identity joining the meeting, and **works for ended meetings too**:
...
| **Agent actually joining / leaving** a meeting | `vc meeting-join` / `vc meeting-leave` | [`lark-vc-agent`](../lark-vc-agent/SKILL.md) |
...
>
> **File-to-notes boundary**: If the user provides a local audio/video file and wants notes, transcripts, summaries, todos, or chapters, the entry point should first go through the [lark-minutes](../lark-minutes/SKILL.md) upload flow to generate a `minute_url` / `minute_token`, then return to `vc-notes` with `minute_tokens` to retrieve the content artifacts.
>
> **Special case**: When the user asks "what meetings do I have today", use `vc-search` to query meetings already held today, and at the same time use the lark-calendar skill to query meetings not yet started today; organize both and display them together.
...
- When using `vc-notes`, MUST read [references/lark-vc-notes.md](references/lark-vc-notes.md) to understand query parameters, artifact types (including minutes keywords), and return value structure.
...
```javascript
lark_api_search({ query: 'vc.<resource>.<method>' })  // Check the parameter structure before calling an API
lark_api({ tool: 'vc', op: '<resource>.<method>', args: { ... } })  // Call the API
```
...
### minutes (cross-domain, see [lark-minutes](../lark-minutes/SKILL.md))
  ...
  - `get` — Get basic Minutes info (title, duration, cover); to query note **content**, use `vc-notes` with `minute_tokens`
