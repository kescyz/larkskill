...
lark_api({ tool: 'calendar', op: 'update', args: {
  event_id: '<EVENT_ID>',
  add_attendee_ids: 'ou_aaa,ou_bbb,omm_room'
} })

// 移除参会人和会议室
lark_api({ tool: 'calendar', op: 'update', args: {
  event_id: '<EVENT_ID>',
  remove_attendee_ids: 'ou_aaa,omm_room'
} })
...
- 对 `summary`、`description`，CLI 以“是否显式传入该字段”判断是否更新，而不是以“值是否为空”判断；如果显式传入空字符串，会把对应字段清空。
...
如需更新 `location`（地理位置，不含会议室位置）、`visibility`（日程公开范围）、自定义 `reminders`（提醒设置）、自定义 `attendee_ability`（参与人权限）、自定义 `free_busy_status`（日程忙闲状态）、`color`（颜色）、附件、视频会议信息、全天日程，或在新增参会人时配置可选参加状态 等高级参数，请改用完整的 API 命令。建议先通过 `lark_api_search({ query: 'calendar.events.patch' })`、`lark_api_search({ query: 'calendar.event.attendees.create' })`、`lark_api_search({ query: 'calendar.event.attendees.batch_delete' })` 查看完整参数定义。
