...
> 3. Do NOT default to a full scan for time-bounded queries.
>
> Only do a full scan from the beginning when:
...
lark_api({ tool: 'task', op: 'get-related-tasks', args: {} })

// List incomplete related tasks
lark_api({ tool: 'task', op: 'get-related-tasks', args: { include_complete: false } })

// Show only tasks created by me
lark_api({ tool: 'task', op: 'get-related-tasks', args: { created_by_me: true } })
```
...
2. Execute `lark_api({ tool: 'task', op: 'get-related-tasks', args: { ... } })`
