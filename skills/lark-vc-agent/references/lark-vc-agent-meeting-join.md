...
本 skill 对应 shortcut：`lark_api({ tool: 'vc', op: 'meeting-join' })`（调用 `POST /open-apis/vc/v1/bots/join`）。
...
// 指定会议号 + 密码
lark_api({ tool: 'vc', op: 'meeting-join', args: { meeting_number: '123456789', password: '8888' } })
```
...
`meeting_number` 必须是 9 位纯数字，否则会校验报错：
...
| 字段 | 说明 |
|------|------|
| `meeting.id` | 会议 ID（可后续传给 `meeting-leave` 的 `meeting_id`） |
...
- 入会会让机器人立即出现在参会列表；若要回退，直接调用 `meeting-leave` 即可。
