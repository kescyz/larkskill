...
本 skill 对应 shortcut：`lark_api({ tool: 'vc', op: 'meeting-events' })`（调用 `GET /open-apis/vc/v1/bots/events`）。

## 命令

```js
// 默认用法：全量拉取当前可见事件
lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '69xxxxxxxxxxxxx28', page_all: true, format: 'pretty' } })

// 指定时间范围，并拉全该时间窗内当前可见事件
lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '69xxxxxxxxxxxxx28', start: '2026-04-17T15:00:00+08:00', end: '2026-04-17T16:00:00+08:00', page_all: true, format: 'pretty' } })

// 基于上一次保存的 page_token 继续查新增事件
lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '69xxxxxxxxxxxxx28', page_token: '<last_page_token>', page_all: true, format: 'pretty' } })
```
...
| `format` | 否 | 输出格式：pretty（本 skill 推荐默认） / json |
...
**不要**把 9 位会议号（`meeting_number`）传给这个命令。
...
```js
// 先入会
lark_api({ tool: 'vc', op: 'meeting-join', args: { meeting_number: '123456789' } })

// 记录返回的 meeting.id

// 再查询事件
lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '<meeting.id>' } })
```
...
- **先分清两层默认值**：
  - 接口本身：不传 `page_all` 时，只查 1 页。
  - 本 skill 的默认策略：除非用户明确要求只看一页，或你确实需要控制返回体大小，否则默认**必须主动带 `page_all: true`**，把当前可见事件尽量一次拉全。
- 传 `page_all: true`：开启自动分页，直到没有更多页面为止。
- `page_all: true` 时，固定使用最大 `page_size=100`。

执行准则：

- **默认调用模板**：`lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '<meeting.id>', page_all: true, format: 'pretty' } })`
- 如果你发现自己执行成了不带 `page_all` 的单页查询，而响应里又出现 `has_more=true` / `more available` / 非空 `page_token`，应立刻意识到这只是部分结果。
- 遇到上述情况，默认补救方式是继续使用返回的 `page_token` 续拉，例如：`lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '<meeting.id>', page_token: '<returned_page_token>', page_all: true, format: 'pretty' } })`
...
**选型原则**：只要目标是告诉用户”发生了什么”，默认就用 `page_all: true, format: 'pretty'`；只有在需要完整原始消息流和结构化字段时，才改用 `format: 'json'`。
...
- 这类问题默认先用 `lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '<meeting.id>', page_all: true, format: 'json' } })` 拉取最新事件流。
- 如果事件中出现共享文档线索，例如：
  - `magic_share_started`
  - `share_doc.title`
  - `share_doc.url`
...
- 如果没有使用 `page_all`，但出现了上述分页信号，默认应继续用返回的 `page_token` 拉下一页，而不是直接结束。只有在用户明确不要继续翻页时，才可以停止并明确说明当前结果不完整。
...
| 输入参数 | 获取方式 |
|---------|---------|
| `meeting_id` | `meeting-join` 返回的 `meeting.id`；或 `search` 结果中的 `id` |
...
// 第 2 步：查询事件流
lark_api({ tool: 'vc', op: 'meeting-events', args: { meeting_id: '<meeting.id>', page_all: true, format: 'pretty' } })
```

### 场景 2：过滤某段时间内的事件

```js
lark_api({ tool: 'vc', op: 'meeting-events', args: {
...
| `10005 bot is not in meeting` | bot 从未真实入会该会议；或会议已结束但 bot 从未在会中出现过 | 先用 `lark_api({ tool: 'vc', op: 'meeting-join', args: { meeting_number: '<9位号>' } })` 真实入会再查；如果会议已经结束且当时 bot 没进过会，本接口也拉不到数据。**如果只是想看参会人快照，改用 `lark_api({ tool: 'vc', op: 'meeting-get', args: { meeting_id: '<meeting.id>', with_participants: true } })`**（不依赖 bot 身份参会） |
| `20001 meeting_status_MEETING_END` | 会议已结束且已超出后端允许的 5 分钟宽限窗口 | 本接口不再适合继续拉取事件。若要拿纪要文档或逐字稿 token，用 `lark_api({ tool: 'vc', op: 'notes', args: { meeting_ids: ['<meeting.id>'] } })`；若要拿 AI 产物（summary / todos / chapters）或导出逐字稿文件，先用 `lark_api({ tool: 'vc', op: 'recording', args: { meeting_ids: ['<meeting.id>'] } })` 拿 `minute_token`，再用 `lark_api({ tool: 'vc', op: 'notes', args: { minute_tokens: ['<minute_token>'] } })`；参会人请用 `lark_api({ tool: 'vc', op: 'meeting-get', args: { meeting_id: '<meeting.id>', with_participants: true } })` |
...
- 如果会议已经结束，不要卡在 `meeting-events`：  
  - 想拿纪要文档或逐字稿 token：用 `lark_api({ tool: 'vc', op: 'notes', args: { meeting_ids: ['<meeting.id>'] } })`
  - 想拿 AI 产物（summary / todos / chapters）或导出逐字稿文件：先用 `lark_api({ tool: 'vc', op: 'recording', args: { meeting_ids: ['<meeting.id>'] } })` 拿 `minute_token`，再用 `lark_api({ tool: 'vc', op: 'notes', args: { minute_tokens: ['<minute_token>'] } })`
- 事件列表是否完整，取决于 bot 何时入会、何时离会，以及后端当前可见的会中事件范围。对于已结束会议，通常只在**结束后 5 分钟内**、且 bot **曾经在会中**时还能继续拉到事件。
- 查询"谁参加过某会议"请用 `lark_api({ tool: 'vc', op: 'meeting-get', args: { meeting_id: '<id>', with_participants: true } })`——这是参会人**快照** API，不依赖 bot 是否参会，对已结束会议也可查；**不要** 用 `meeting-events` 做参会人查询。
