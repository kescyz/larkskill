...
```js
lark_api({ tool: 'wiki', op: 'node-get', args: {
  node_token: '<node_token | obj_token | Lark URL>',
  // obj_type: '<doc|docx|sheet|bitable|mindnote|slides|file>',
  // space_id: '<space_id>',
  as: 'user'
} })
```
...
|------|------|----------|---------|-------------|
| `node_token` | string | **Yes** | — | `node_token`, cloud-doc `obj_token`, or a Lark URL embedding one (e.g. `https://feishu.cn/wiki/<token>` or `https://feishu.cn/docx/<token>`). Matches the `node_token` naming used by sibling `+node-delete` / `+node-copy` / `+move`. |
| `token` | string | — (deprecated) | — | Deprecated original name; still accepted for backward compatibility but emits a `Flag --token has been deprecated, use --node-token instead` warning on stderr. New scripts should use `node_token`. |
