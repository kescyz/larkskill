...
lark_api({ tool: 'im', op: 'chat-list', args: {} })

// Sort by recent activity (most recently active first)
lark_api({ tool: 'im', op: 'chat-list', args: { sortType: 'ByActiveTimeDesc' } })

// Limit page size
lark_api({ tool: 'im', op: 'chat-list', args: { pageSize: 50 } })

// Pagination
lark_api({ tool: 'im', op: 'chat-list', args: { pageToken: 'xxx' } })

// Drop muted chats (user identity only)
lark_api({ tool: 'im', op: 'chat-list', args: { excludeMuted: true } })

// JSON output
lark_api({ tool: 'im', op: 'chat-list', args: { format: 'json' } })

// Preview the request without executing it
lark_api({ tool: 'im', op: 'chat-list', args: { dryRun: true } })

// Include p2p single chats (user identity only) — comma form
lark_api({ tool: 'im', op: 'chat-list', args: { as: 'user', types: 'p2p,group' } })

// Same, using array instead of CSV
lark_api({ tool: 'im', op: 'chat-list', args: { as: 'user', types: ['p2p', 'group'] } })

// Only p2p single chats (user identity only)
lark_api({ tool: 'im', op: 'chat-list', args: { as: 'user', types: 'p2p' } })
```
...
```javascript
lark_api({ tool: 'im', op: 'chat-list', args: { sortType: 'ByActiveTimeDesc', pageSize: 10 } })
```

### Scenario 2: List my non-muted chats sorted by activity

```javascript
lark_api({ tool: 'im', op: 'chat-list', args: { sortType: 'ByActiveTimeDesc', excludeMuted: true } })
```
...
```javascript
// First page
lark_api({ tool: 'im', op: 'chat-list', args: { pageSize: 100, format: 'json' } })
// Continue with page_token while has_more=true
lark_api({ tool: 'im', op: 'chat-list', args: { pageSize: 100, pageToken: '<token_from_previous>', format: 'json' } })
```
