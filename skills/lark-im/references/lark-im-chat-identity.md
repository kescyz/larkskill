...
- **Owner-only actions such as owner transfer:** require the actual owner to complete UAT authorization via `lark_auth_login`, then perform the action as that owner.
...
1. Create the group with the bot first, excluding invisible users: `lark_api({ tool: 'im', op: 'chat-create', args: { name: 'Group Name' } })`
