...
2. A bot created the group and an owner open_id **was** specified -> the owner is that user (`as: user`)
...
- **Owner-only actions such as owner transfer:** require the actual owner to authenticate, then perform the action as that owner.
...
If a bot creates a group and `user_id_list` includes users who are mutually invisible to the bot, the entire request fails with 232043. Use two steps instead:

1. Create the group with the bot first, excluding invisible users
