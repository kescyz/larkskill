...
This skill maps to the shortcut: `lark_api({ tool: 'im', op: 'chat-messages-list' })` (internally calls `GET /open-apis/im/v1/messages`, and automatically resolves the p2p chat_id when needed).
...
lark_api({ tool: 'im', op: 'chat-messages-list', args: { user_id: 'ou_xxx' } })

// Specify a time range (ISO 8601)
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chat_id: 'oc_xxx', start: '2026-03-10T00:00:00+08:00', end: '2026-03-11T00:00:00+08:00' } })

// Specify a time range (date only)
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chat_id: 'oc_xxx', start: '2026-03-10', end: '2026-03-11' } })

// Control sort order
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chat_id: 'oc_xxx', sort: 'asc' } })
```
...
| `user_id` | One of two | Specify a DM conversation by the other user's open_id (`ou_xxx`); p2p chat_id is resolved automatically. Requires user identity (`as: 'user'`); not supported with bot identity |
...
|---------|-------------|------|
| Image | `[Image: img_xxx]` | Download manually with `messages-resources-download` (`type: 'image'`) |
| File | `<file key="file_xxx" .../>` | Download manually with `messages-resources-download` (`type: 'file'`) |
| Audio | `<audio key="file_xxx" .../>` | Download manually with `messages-resources-download` (`type: 'file'`) |
| Video | `<video key="file_xxx" .../>` | Download manually with `messages-resources-download` (`type: 'file'`) |

## Thread Expansion (`thread_id`)

In JSON output, a message may contain a `thread_id` (`omt_xxx`) field, which means the message has replies in a thread. Use [`threads-messages-list`](lark-im-threads-messages-list.md) to inspect replies in that thread:
...
|------|------|
| You need context | Call `threads-messages-list` with `sort: 'desc'` for the discovered thread_id to inspect recent replies |
| The user asks for the "full discussion" | Use `threads-messages-list` with `sort: 'asc'`, then paginate if needed |
...
`chat-messages-list` returns `has_more` and `page_token` when more data is available. Use `page_token` to continue:

```js
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chat_id: 'oc_xxx', page_token: '<PAGE_TOKEN>' } })
```

You can also fall back to the generic API:

```js
lark_api({ tool: 'api', op: 'GET', args: { path: '/open-apis/im/v1/messages', params: 'container_id_type=chat&container_id=oc_xxx&page_size=50&page_token=<PAGE_TOKEN>' } })
```
...
| `user_id requires user identity (as: 'user'); use chat_id when calling with bot identity` | `user_id` was used with bot identity | The p2p resolution endpoint requires user identity. Either pass `as: 'user'` or look up the p2p `chat_id` separately and pass it via `chat_id` |
...
3. **For direct messages:** use `user_id` to resolve the p2p chat automatically instead of looking it up manually. This requires user identity (`as: 'user'`); with bot identity, resolve the p2p `chat_id` yourself and pass it via `chat_id`.
4. **For time ranges:** both ISO 8601 and date-only inputs are supported. Date-only is usually simpler.
5. **For full content:** the command returns the complete message body in JSON.
...
7. **Application/bot identity + named group history:** If the user says "使用应用身份/以 bot 身份" and asks to list or read historical messages for a named group, use bot identity for both steps:
   ```js
   lark_api({ tool: 'im', op: 'chat-search', args: { as: 'bot', query: '<chat name keyword>' } })
   lark_api({ tool: 'im', op: 'chat-messages-list', args: { as: 'bot', chat_id: '<chat_id>' } })
   ```
   Do not use `messages-search` with `as: 'bot'`; `messages-search` is user-only. Continue with `page_token` if `has_more=true`.
