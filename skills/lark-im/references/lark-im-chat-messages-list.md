...
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx' } })

// Get direct messages with a user (pass open_id and resolve p2p chat_id automatically)
lark_api({ tool: 'im', op: 'chat-messages-list', args: { userId: 'ou_xxx' } })

// Specify a time range (ISO 8601)
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx', start: '2026-03-10T00:00:00+08:00', end: '2026-03-11T00:00:00+08:00' } })

// Specify a time range (date only)
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx', start: '2026-03-10', end: '2026-03-11' } })

// Control sort order and page size (max 50)
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx', sort: 'asc', pageSize: 20 } })

// Pagination
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx', pageToken: 'xxx' } })

// JSON output
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx', format: 'json' } })
```
...
```javascript
lark_api({ tool: 'im', op: 'chat-messages-list', args: { chatId: 'oc_xxx', pageToken: '<PAGE_TOKEN>' } })
```

You can also fall back to the generic API:

```javascript
lark_api({ method: 'GET', path: '/open-apis/im/v1/messages', params: { container_id_type: 'chat', container_id: 'oc_xxx', page_size: 50, page_token: '<PAGE_TOKEN>' } })
```
...
7. **Application/bot identity + named group history:** If the user says "使用应用身份/以 bot 身份" and asks to list or read historical messages for a named group, use bot identity for both steps:
   ```javascript
   lark_api({ tool: 'im', op: 'chat-search', args: { as: 'bot', query: '<chat name keyword>', format: 'json' } })
   lark_api({ tool: 'im', op: 'chat-messages-list', args: { as: 'bot', chatId: '<chat_id>', pageSize: 50, format: 'json' } })
   ```
   Do not use `messages-search` with `as: 'bot'`; `messages-search` is user-only. Continue with `pageToken` if `has_more=true`.
