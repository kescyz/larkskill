...
This skill maps to the shortcut: `lark_api({ tool: 'im', op: 'messages-mget' })` (internally calls `GET /open-apis/im/v1/messages/mget`).

## Commands

```javascript
// Fetch a single message
lark_api({ tool: 'im', op: 'messages-mget', args: { messageIds: 'om_xxx' } })

// Fetch multiple messages in batch (comma-separated)
lark_api({ tool: 'im', op: 'messages-mget', args: { messageIds: 'om_aaa,om_bbb,om_ccc' } })

// JSON output
lark_api({ tool: 'im', op: 'messages-mget', args: { messageIds: 'om_aaa,om_bbb', format: 'json' } })

// Preview the request without executing it
lark_api({ tool: 'im', op: 'messages-mget', args: { messageIds: 'om_aaa', dryRun: true } })
```
...
```javascript
lark_api({ tool: 'im', op: 'messages-mget', args: { messageIds: 'om_xxx', format: 'json' } })
```
...
lark_api({ tool: 'im', op: 'messages-mget', args: { messageIds: 'om_aaa,om_bbb' } })
```
