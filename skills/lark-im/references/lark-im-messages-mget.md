...
lark_api({ tool: 'im', op: 'messages-mget', args: { message_ids: 'om_xxx' } })

// Fetch multiple messages in batch (comma-separated)
lark_api({ tool: 'im', op: 'messages-mget', args: { message_ids: 'om_aaa,om_bbb,om_ccc' } })
```
...
lark_api({ tool: 'im', op: 'messages-mget', args: { message_ids: 'om_aaa,om_bbb' } })
```
...
1. **The response carries full content:** the command returns the complete message body in JSON.
